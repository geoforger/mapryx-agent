...
This deployment is protected by the Worker before the React app, agent API, or WebSocket endpoint is served.

Set at least one secret before deploying:

```bash
wrangler secret put AGENT_ACCESS_PASSWORD
wrangler secret put AGENT_SESSION_SECRET
```

`AGENT_ACCESS_PASSWORD` enables the browser login page at `/auth/login`. `AGENT_SESSION_SECRET` signs the login cookie; use a long random value.

For machine-to-machine access, set a bearer token instead or in addition:

```bash
wrangler secret put AGENT_AUTH_TOKEN
```

Then call the Worker with:

```bash
Authorization: Bearer <token>
```

If neither `AGENT_ACCESS_PASSWORD` nor `AGENT_AUTH_TOKEN` is configured, the Worker returns `503` instead of serving the public app.
