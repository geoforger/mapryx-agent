...
const SESSIONS_KEY = "mapryx-agent-sessions";

function loadSessions(): { id: string; name: string; createdAt: number }[] {
  try {
...
function Chat({ sessionId, sessionName }: { sessionId: string; sessionName: string }) {
  ...
  const {
    ...
    onToolCall: async (_event) => {
      // Frontend tool handlers will be wired here when Mapryx tools are integrated
    }
  });
            ...
            <Empty
              ...
              contents={
                <div className="flex flex-wrap justify-center gap-2">
                  {[
                    "What geospatial operations can you perform?",
                    "Describe the layers currently loaded in Mapryx",
                    "Schedule a daily data refresh at 9am",
...
function AppInner() {
  const [sessions, setSessions] = useState(() => {
    ...
    const initial = { id: "default", name: "Default", createdAt: Date.now() };
  ...
  const handleCreate = () => {
    ...
    const next = [...sessions, { id, name, createdAt: Date.now() }];
    setSessions(next);
    saveSessions(next);
    setActiveId(id);
  };
